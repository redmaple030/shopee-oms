            ...
            for label in self.field_keys:
                ...
                for opt in options:
                    ...
                    if label in opt: 
                        cb.set(opt) 
                        break
                    if label == "廠商名稱" and ("商店" in h_low or "公司" in h_low or "店名" in h_low or "名稱" in h_low):
                         cb.set(opt)
                         break
                    if label == "通路" and ("來源" in h_low or "平台" in h_low):
                         cb.set(opt)
                         break
                    if label == "統編" and ("統一編號" in h_low or "tax" in h_low): 
                        cb.set(opt)
                        break
                    if label == "聯絡人" and ("對口" in h_low or "負責人" in h_low): 
                        cb.set(opt)
                        break
    ...
    def execute_import(self):
        ...
        for _, row in self.import_raw_df.iterrows():
            try:
                ...
                if not v_name or v_name.lower() == "nan":
                    continue
