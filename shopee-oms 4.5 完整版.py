...
class SalesApp:
    ...
    def refresh_vendor_live_score(self, vendor_name):
        ...
        try:
            ...
            v_data = df_h[v_mask].copy()

            if v_data.empty:
                self.var_v_system_score.set("0.0 (質:0% / 備:0d / 運:0d)")
                self.var_v_leadtime.set("尚無紀錄")
                return

            # --- 數據清洗：轉換日期，錯誤的變 NaT (空值) ---
            ...
            q_rate = round((1 - (total_defects / total_qty)) * 100, 1) if total_qty > 0 else 100

            # --- 4. 計算滿足率 ---
            orig_s = pd.to_numeric(v_data.get('原始預計數量', qty_s), errors='coerce').fillna(0)
            total_orig = orig_s.sum()
            f_rate = round((total_qty / total_orig) * 100, 1) if total_orig > 0 else 100
            ...
            # --- 7. UI 更新 (確保不輸出 nan 字串) ---
            # 如果計算出來是 NaN，則強制轉為 0
            import math
            def clean_nan(val):
                return 0 if (val is None or (isinstance(val, float) and math.isnan(val))) else val

            final_mixed_score = clean_nan(final_mixed_score)
            avg_prep = clean_nan(avg_prep)
            avg_transit = clean_nan(avg_transit)
            ...
            print(f"評分系統出錯: {e}")
            self.var_v_system_score.set("計算出錯 (請檢查資料格式)")
        ...
        def perform_save():
            try:
                ...
                for item in batch_list:
                    ...
                    if new_logi: df_track.at[t_idx, '物流追蹤'] = new_logi
...
class SalesApp:
    ...
    def load_purchase_tracking(self):
        ...
        try:
            ...
            # --- [核心修正：載入時自動補零] ---
            # pd.to_numeric 會把非數字轉為 NaN，再用 fillna(0) 把 NaN 全部變回 0
            df['海關稅金'] = pd.to_numeric(df.get('海關稅金', 0), errors='coerce').fillna(0)
            df['分攤運費'] = pd.to_numeric(df.get('分攤運費', 0), errors='coerce').fillna(0)
            ...
            for idx, row in df.iterrows():
                self.tree_pur_track.insert("", "end", text=str(idx), values=(
                    ...
                    row.get('數量', 0),
                    row.get('進貨單價', 0),
                    row.get('海關稅金', 0), # 現在這裡絕對會是數字
                    row.get('分攤運費', 0), # 現在這裡絕對會是數字
                    row.get('物流追蹤', '待發貨')
                ))
        except Exception as e:
            print(f"載入追蹤清單出錯: {e}")
