...
class SalesApp:
    ...
    def add_to_pur_cart_batch(self):
        ...
        for idx in selections:
            ...
            # 建立預設資料 (數量 1)
            tax = round(default_cost * 0.05, 2) if self.var_pur_tax_enabled.get() else 0.0
            total = default_cost # 因為數量是 1

            self.pur_cart_data.append({
                "name": p_name, "qty": 1, "cost": default_cost, "tax": tax, "total": total
            })
            self.tree_pur_cart.insert("", "end", values=(p_name, 1, default_cost, tax, total))

        self.update_pur_cart_total()
        
        # 優化體驗：搜尋框自動反白，方便直接搜尋下一組
    ...
    def on_pur_cart_double_click(self, event):
        ...
        def confirm_edit(e=None):
            try:
                new_q = int(ent_qty.get())
                new_c = float(ent_cost.get())
                new_total = new_q * new_c
                new_tax = round(new_total * 0.05, 2) if self.var_pur_tax_enabled.get() else 0.0
                
                # 更新記憶體與介面
                self.pur_cart_data[idx].update({"qty": new_q, "cost": new_c, "tax": new_tax, "total": new_total})
                self.tree_pur_cart.item(item_id, values=(current['name'], new_q, new_c, new_tax, new_total))
                self.update_pur_cart_total()
                win.destroy()
            except: messagebox.showerror("錯誤", "請輸入正確數字格式")
    ...
    def refresh_vendor_live_score(self, vendor_name):
        ...
        try:
            ...
            total_qty = qty_s.sum()
            total_defects = defect_s.sum()
            q_rate = round((1 - (total_defects / total_qty)) * 100, 1) if total_qty > 0 else 0

            # 4. 到貨滿足率
            orig_s = pd.to_numeric(v_data.get('原始預計數量', qty_s), errors='coerce').fillna(0)
            total_orig = orig_s.sum()
            f_rate = round((total_qty / total_orig) * 100, 1) if total_orig > 0 else 0

            # 5. 系統評分 (權重維持您的設定)
            score_prep = max(100 - (max(avg_prep - 3, 0) * 10), 0)
            score_transit = max(100 - (max(avg_transit - 5, 0) * 10), 0)
            system_score = (q_rate * 0.4) + (score_prep * 0.3) + (f_rate * 0.2) + (score_transit * 0.1)

            # 6. 混合人為星等
            try: manual_stars = int(self.var_v_manual_adj.get())
            except: manual_stars = 5
            final_mixed_score = (system_score * 0.8) + (manual_stars * 20 * 0.2)

            # 7. UI 更新
            display_text = f"{round(final_mixed_score, 1)} (到貨合格率:{int(q_rate)}% / 備貨時間:{avg_prep}d / 運輸時效:{avg_transit}d)"
            ...
            print(f"評分系統出錯: {e}")
            self.var_v_system_score.set("計算出錯")
    ...
    def generate_procurement_report(self):
        ...
        try:
            ...
            try:
                ...
                s_multiplier = float(self.var_safety_multiplier.get())
                cover_days = float(self.var_days_to_cover.get())
            except:
                v_threshold, s_multiplier, cover_days = 0.1, 1.0, 30.0
            ...
            for _, row in df_prods.iterrows():
                ...
                days_diff = (now - st_date).days
                velocity = total_sold / max(days_diff, 1) # 至少除以 1 天

                # 補貨計算
                target_inventory = (velocity * cover_days) + (base_safety * s_multiplier)
                import math
                suggest_qty = math.ceil(max(target_inventory - curr_stock, 0))
                ...
                self.tree_procure.insert("", "end", values=(
                    p_name, int(curr_stock), round(base_safety * s_multiplier, 1), 
                    f"{round(velocity, 2)}件/日", status, int(suggest_qty)
                ), tags=(tag,))
...
# [新增這個輔助函式]：統一計算並更新介面
    ...
    def action_update_pur_logistics(self):
        """ 
        物流維護 V6.2 (索引精準批次更新版)
        支援單筆數量校準，以及多筆複選同步狀態功能
        """
        selected_items = self.tree_pur_track.selection()
        if not selected_items:
            messagebox.showwarning("提示", "請先選擇一筆或多筆商品項目")
            return

        # 1. 蒐集選中項目的資訊 (Row Index 是關鍵)
        batch_list = []
        for item_id in selected_items:
            item_data = self.tree_pur_track.item(item_id)
            # text 裡面存的是 load_purchase_tracking 時寫入的原始 Excel 索引 (idx)
            batch_list.append({
                "df_idx": int(item_data['text']), 
                "p_name": str(item_data['values'][2]).strip(),
                "pur_id": str(item_data['values'][0]).strip(),
                "qty": int(item_data['values'][3])
            })

        is_batch = len(batch_list) > 1
        win = tk.Toplevel(self.root)
        win.title("📦 批次物流狀態更新" if is_batch else f"物流維護 - {batch_list[0]['p_name']}")
        ...
        # --- 頂部顯示 ---
        if is_batch:
            ttk.Label(body, text=f"🔔 目前已選取 {len(batch_list)} 項商品", font=("", 10, "bold"), foreground="red").pack(pady=5)
        else:
            ttk.Label(body, text=f"商品: {batch_list[0]['p_name']}", font=("", 10, "bold")).pack(pady=5)

        # --- 數量區塊 (僅在單筆時顯示) ---
        var_actual_qty = tk.IntVar(value=batch_list[0]['qty'])
        var_defects = tk.IntVar(value=0)
        if not is_batch:
            qty_frame = ttk.LabelFrame(body, text="📦 數量與瑕疵驗收", padding=10)
            qty_frame.pack(fill="x", pady=10)
            ttk.Label(qty_frame, text="實際收到數量:").pack(anchor="w")
            ttk.Entry(qty_frame, textvariable=var_actual_qty).pack(fill="x", pady=2)
            ttk.Label(qty_frame, text="瑕疵損壞數量:").pack(anchor="w")
            ttk.Entry(qty_frame, textvariable=var_defects).pack(fill="x", pady=2)
        else:
            ttk.Label(body, text="*(批次模式下僅更新物流狀態與單號，不連動數量)", foreground="gray", font=("", 9)).pack()

        # --- 物流狀態與單號 ---
        logi_frame = ttk.LabelFrame(body, text="🚚 物流階段追蹤", padding=10)
        logi_frame.pack(fill="x", pady=10)
        
        ttk.Label(logi_frame, text="變更階段為:", foreground="blue").pack(anchor="w")
        var_status = tk.StringVar(value="廠商已發貨")
        status_combo = ttk.Combobox(logi_frame, textvariable=var_status, state="readonly")
        status_combo['values'] = ("待出貨", "廠商已發貨", "貨到集運倉", "集運倉已發貨", "抵達台灣海關", "國內配送中", "已完成入庫")
        status_combo.pack(fill="x", pady=5)

        ttk.Label(logi_frame, text="物流單號 (複選時將套用至所有項):").pack(anchor="w", pady=(10,0))
        var_logi_id = tk.StringVar()
        ttk.Entry(logi_frame, textvariable=var_logi_id).pack(fill="x", pady=5)
        ...
        def perform_save():
            try:
                ...
                # --- 核心：清潔資料，防止 nan 寫入文字欄位 ---
                text_cols = ['物流狀態', '物流追蹤', '備註', '進貨單號', '商品名稱', '時間_廠商出貨', 
                             '時間_抵達集運倉', '時間_集運倉出貨', '時間_抵達台灣海關', '入庫日期']
                
                for df in [df_track, df_hist]:
                    for col in text_cols:
                        if col not in df.columns: df[col] = ""
                        # 先把真正的空值處理掉，避免轉字串變 "nan"
                ...
                for item in batch_list:
                    ...
                    # 1. 更新追蹤表 (使用 Row Index 最精準)
                    df_track.at[t_idx, '物流狀態'] = new_status
                    if new_logi:
                        df_track.at[t_idx, '物流追蹤'] = f"'{new_logi}" # 加單引號是為了保護 Excel 格式
                    ...
                    if not is_batch:
                        ...
                        # 重算總額
                        price = pd.to_numeric(df_track.at[t_idx, '進貨單價'], errors='coerce')
                        df_track.at[t_idx, '進貨總額'] = var_actual_qty.get() * price

                    # 紀錄對應的時間節點
                    time_map = {"廠商已發貨": "時間_廠商出貨", "貨到集運倉": "時間_抵達集運倉", 
                                "集運倉已發貨": "時間_集運倉出貨", "抵達台灣海關": "時間_抵達台灣海關", "已完成入庫": "入庫日期"}
                    col_name = time_map.get(new_status)
                    if col_name: df_track.at[t_idx, col_name] = today_str

                    # 2. 同步更新歷史總表 (df_hist) - 採「單號+品名」比對
                    h_mask = (df_hist['進貨單號'].str.replace("'", "").str.strip() == item['pur_id']) & \
                             (df_hist['商品名稱'].str.strip() == item['p_name'])
                    
                    if not df_hist[h_mask].empty:
                        df_hist.loc[h_mask, '物流狀態'] = new_status
                        if new_logi: df_hist.loc[h_mask, '物流追蹤'] = new_logi
                        if col_name: df_hist.loc[h_mask, col_name] = today_str
                        if not is_batch:
                            df_hist.loc[h_mask, '數量'] = var_actual_qty.get()
                            df_hist.loc[h_mask, '瑕疵數量'] = var_defects.get()

                # 存檔
                if self._universal_save({SHEET_PUR_TRACKING: df_track, SHEET_PURCHASES: df_hist}):
                    messagebox.showinfo("成功", f"批次處理完成！已更新 {len(batch_list)} 筆項目。")
                    self.load_purchase_tracking()
                    win.destroy()
        ...
        ttk.Button(body, text="💾 執行批次更新並存檔", command=perform_save).pack(pady=20, fill="x")
    ...
    def action_track_complete_order(self):
        ...
        try:
            ...
            # --- [新增：確保結案前填滿該單的所有資訊] ---
            # 這樣統計時，每一行商品都有對應的日期和買家，才不會算錯
            info = self._get_full_order_info(df_track, order_id)
            for col, val in info.items():
                rows_to_finish[col] = val  # 強制將日期、買家等資訊填入每一列

            # 合併到銷售紀錄
            df_sales_combined = pd.concat([df_sales, rows_to_finish], ignore_index=True)

            # --- [核心修正：自動按日期與編號排序] ---
            # 確保銷售紀錄永遠是最新日期在最下面(或最上面)
            df_sales_combined['tmp_date'] = pd.to_datetime(df_sales_combined['日期'], errors='coerce')
            df_sales_combined = df_sales_combined.sort_values(by=['tmp_date', '訂單編號'], ascending=[True, True])
            df_sales_combined.drop(columns=['tmp_date'], inplace=True)
            ...
            if success:
                messagebox.showinfo("成功", f"訂單 {order_id} 已結案並自動排序優化！")
                self.load_tracking_data()
                self.calculate_analysis_data()
        except Exception as e: 
    ...
    def _universal_save(self, updates_dict):
        """ 強化版萬用存檔引擎：防止分頁消失，並強制保護 ID 與物流單號為文字格式 """
        try:
            all_data = {}
            ...
            # 3. 核心數據清洗：防止長數字變成浮點數 (.0) 或科學記號
            # 定義需要被強制轉為純文字且加上單引號保護的欄位
            text_protection_cols = ['訂單編號', '進貨單號', '物流追蹤', '商品編號', '物流單號']

            for sn, df in all_data.items():
                if df is None or df.empty: continue
                
                for col in text_protection_cols:
                    if col in df.columns:
                        # 定義清洗邏輯：轉字串 -> 去掉 .0 -> 去掉 nan -> 補上單引號
                        def force_text_format(x):
                            ...
                            # 處理 Pandas 自動產生的 .0 (針對長數字浮點數)
                            if s.endswith('.0'):
                                s = s[:-2]
                            
                            # 確保開頭只有一個單引號，防止重複添加變成 ''12345
                            s = s.lstrip("'")
                            return f"'{s}"

                        df[col] = df[col].apply(force_text_format)

            # 4. 寫回 Excel (按照標準順序排列分頁)
            with pd.ExcelWriter(FILE_NAME, engine='openpyxl') as writer:
                standard_order = [
            ...
            import traceback
            traceback.print_exc() # 在終端機印出詳細報錯，方便 Debug
